        ...
        <?php if ($product->image){?>
            <div class="image_block">
                ...
                <a href="<?php print $product->product_link?>">
                    <img class="jshop_img uk-teaser-image" src="<?php print $product->image?>" alt="<?php print htmlspecialchars($product->name);?>" title="<?php print htmlspecialchars($product->name);?>"  />
                </a>
            </div>
        <?php }?>
    ...
    <div class="uk-width-1-1 uk-teaser-description">
        ...
        <?php/* if ($product->manufacturer->name){?>
            <div class="manufacturer_name uk-grid uk-grid-collapse">
                ...
                <div class="uk-width-3-4"><?php print $product->manufacturer->name?></div>
            </div>
        <?php }*/?>
            ...
            <div class="extra_fields uk-width-1-1 uk-grid uk-grid-collapse">
                <?php foreach($product->extra_field as $extra_field){?>
                    <div class=" uk-width-1-1 uk-grid uk-grid-collapse">
                        <div class="label-name uk-width-1-4"><?php print $extra_field['name'];?>:</div>
                        <div class="data uk-width-3-4"><?php print $extra_field['value'];?></div>
                    </div>
                <?php }?>
				<?php /*if ($product->image){?>
					<div class="image_block">
						...
						<?php }?>
						<a class="lightbox" id="main_image_full_<?php print $product->id?>" href="<?php print $product->image?>" data-lightbox-type="image" data-uk-lightbox="on">
							<img class="jshop_img uk-teaser-image" id="main_image_full_<?php print $product->id?>" src="<?php print $product->image?>" alt="<?php print htmlspecialchars($product->name);?>" title="<?php print htmlspecialchars($product->name);?>"  />
						</a>
					</div>
				<?php }?>//конец блока с изображением
            </div>            
        <?php } */?>
        ...
        <div class="buttons">
			...
			<?php/*
            <a class="uk-button uk-button-small uk-teaser-readmore uk-position-absolute" href="<?php print $product->product_link?>">
