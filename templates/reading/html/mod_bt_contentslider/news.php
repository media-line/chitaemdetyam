...
if(count($list)>0){?>
<div id="btcontentslider<?php echo $module->id; ?>" style="display:none;width:<?php echo $moduleWidthWrapper;?>" class="uk-newsblock bt-cs<?php echo $moduleclass_sfx? ' bt-cs'.$params->get('moduleclass_sfx'):'';?>">
	...
	?>
	<div class="uk-newsblock-title uk-text-center uk-h1 uk-text-bold uk-text-black uk-text-uppercase uk-position-relative" style="width:<?php echo $moduleWidth;?>">
		
		<span><?php echo $params->get('content_title') ?> </span>                    
		<div class="uk-inline-block uk-margin-left">
			<?php if($params->get('content_link')) {?>
				<a class="uk-button uk-button-news" href="<?php echo $params->get('content_link');?>">
					<?php echo JText::_("TPL_NEWS_ALLNEWS_LINK"); ?>
				</a>
			<?php } ?>
		</div>
	</div>
			...
			<div class="bt-row <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>"  style="width:<?php echo $itemWidth;?>%" >
				...
				<?php if($show_category_name_as_link) : ?>
					...
					<a target="<?php echo $openTarget; ?>"
						class="uk-news-img bt-image-link<?php echo $modal? ' modal':''?>"
					...
					<?php if( $showTitle ): ?>
					<a class="uk-news-title" target="<?php echo $openTarget; ?>"
